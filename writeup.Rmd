...
# Load the data
covid_data <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/4360e50239b4eb6b22f3a1759323748f36752177/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv")

# Aggregate data at the country level
country_cases <- covid_data %>%
  group_by(`Country/Region`) %>%
  summarize(total_cases = sum(!is.na(cases)), 
            total_deaths = sum(!is.na(deaths)))
...
country_cases <- country_cases %>%
  mutate(log_total_cases = log(total_cases + 1),  # Adding 1 to avoid log(0)
         log_total_deaths = log(total_deaths + 1))  # Adding 1 to avoid log(0)

# Scatter plot for total cases vs. total deaths
plot1 <- ggplot(country_cases, aes(x = total_cases, y = total_deaths)) +
  geom_point() +
...
plot2 <- ggplot(country_cases, aes(x = log_total_cases, y = log_total_deaths)) +
  geom_point() +
